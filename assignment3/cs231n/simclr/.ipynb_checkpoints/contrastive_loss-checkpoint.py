...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            if i != k:
                denominator_l1 += torch.exp(sim(z_k, out[i]) / tau)
            if i != k + N:
                denominator_l2 += torch.exp(sim(z_k_N, out[i]) / tau)
        
        l_1 = -torch.log((torch.exp(sim(z_k, z_k_N) / tau)) / denominator_l1)
        l_2 = -torch.log((torch.exp(sim(z_k_N, z_k) / tau)) / denominator_l2)
        total_loss += l_1 + l_2
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = pos_pairs.view(len(pos_pairs), 1)
...
def compute_sim_matrix(out):
    ...
    norm = torch.linalg.norm(out, axis=1)
    sim_matrix = sim_matrix / (norm * norm.view(len(norm), 1)) 
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = exponential.sum(axis=1).view(2 * N, 1) # [2N, 1]
    ...
    numerator = torch.cat([torch.exp(sim_pairs / tau), torch.exp(sim_pairs / tau)], dim=0) # [2N, 1]
