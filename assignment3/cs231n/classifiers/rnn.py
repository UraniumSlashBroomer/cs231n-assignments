...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            h, cache_forward = rnn_forward(x, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            h, cache_forward = lstm_forward(x, h0, Wx, Wh, b)
        ...
        # backward pass
        dx, dW_vocab, db_vocab = temporal_affine_backward(dx, cache_temporal_affine)
        
        if self.cell_type == 'rnn':
            dx, dh0, dWx, dWh, db = rnn_backward(dx, cache_forward)
        elif self.cell_type == 'lstm':
            dx, dh0, dWx, dWh, db = lstm_backward(dx, cache_forward)

        dW_embed = word_embedding_backward(dx, cache_embed)
        dx, dW_proj, db_proj = affine_backward(dh0, cache_affine)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'rnn':
            for i in range(max_length):
                x, _ = word_embedding_forward(tokens, W_embed)
