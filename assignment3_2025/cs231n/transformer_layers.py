...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query(query).view(N, S, self.n_head, self.head_dim).moveaxis(1, 2) # [N, H, S, D]
        key = self.key(key).view(N, T, self.n_head, self.head_dim).moveaxis(1, 2) # [N, H, T, D]
        value = self.value(value).view(N, T, self.n_head, self.head_dim).moveaxis(1, 2) # [N, H, T, D]
        
        A = (query @ key.transpose(2, 3)) / math.sqrt(self.head_dim) # [N, H, S, T]
        ...
        output = A @ value # [N, H, S, T], [N, H, T, D] -> [N, H, S, D]
        output = output.moveaxis(1, 2).reshape(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # Идея: разделяем оси H, W на патчи по num_patches_in_line, потом меняем оси и выпрямляем вектора
        
        num_patches_in_line = self.img_size // self.patch_size
        
        x = x.reshape(N, C, num_patches_in_line, self.patch_size, num_patches_in_line, self.patch_size) # [N, C, num_patches_in_line, patch_size, num_patches_in_line, patch_size]
        x = x.permute(0, 1, 2, 4, 3, 5) #  [N, C, num_patches_in_line, num_patches_in_line, patch_size, patch_size]
        x = x.reshape(N, C, self.num_patches, self.patch_size, self.patch_size) # [N, C, num_patches, patch_size, patch_size]
        x = x.permute(0, 2, 1, 3, 4)
        x = x.reshape(N, self.num_patches, C * self.patch_size ** 2)
        out = self.proj(x)
