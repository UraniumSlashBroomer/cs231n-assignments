...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions = self.embedding(captions) # [N, T, W]
        captions = self.positional_encoding(captions) # [N, T, W]
        
        features = self.visual_projection(features) # [N, D] -> [N, W]
        features = features.unsqueeze(1) # [N, W] -> [N, 1, W]
        
        tgt_mask = torch.ones((T, T))
        tgt_mask = torch.tril(tgt_mask, diagonal=0)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.transformer(x, None) # [N, num_patches, E]
        x = x.mean(dim=1)
        logits = self.head(x)
