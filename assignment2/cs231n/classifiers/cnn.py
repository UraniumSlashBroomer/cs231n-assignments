...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params['W2'] = np.random.normal(loc=0.0, scale=weight_scale, size=(input_dim[1] * input_dim[2] * num_filters // 4, hidden_dim)) # Пояснение про размерность: в описании написано, что конв леер не меняет ширину и высоту, то есть на выходе мы получаем фильтр мапы размером input_dim[1] * input_dim[2] * num_filters. После этого мы применяем макспул леер 2x2 со страйдом 2, из-за чего размерность падает в 4 раза
    ...
    def loss(self, X, y=None):
        ...
        dx = dout
        dx, dw, db = affine_backward(dx, cache[2])
        grads['W3'] = dw
        grads['b3'] = db
        dx, dw, db = affine_relu_backward(dx, cache[1])
        grads['W2'] = dw
        grads['b2'] = db
        dx, dw, db = conv_relu_pool_backward(dx, cache[0])
        grads['W1'] = dw
        grads['b1'] = db
